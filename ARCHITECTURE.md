...
This document explains the high-level runtime shape of BubblesTheDev Web Browser version `1.2.15`.
...
Version `1.2.15` keeps the broader browser profile system while keeping the browser local-first.
...
Version `1.2.15` keeps the local AI layer on-device by default while carrying forward the broader accessibility, startup, installer, media-tool, multilingual, and newer profile-system refinements from the earlier `1.1.x` releases.
...
Version `1.2.15` also keeps the expanded browser accessibility layer.
...
Version `1.2.15` keeps the centralized localization manager in the main process and now routes the newer profile and auth UI strings through that same protected pipeline.
...
Version `1.2.15` also keeps the runtime shape where the main window can open sooner while slower background work continues after launch.
