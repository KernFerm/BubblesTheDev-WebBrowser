...
Update note for readers: the original comparison text below was written around version `1.0.65`. The browser has since moved forward to version `1.1.33`, and the addendum sections in this document extend the comparison with the newer local AI, privacy-safe diagnostics, accessibility, installer-behavior, multilingual localization, and panel-polish updates without removing the earlier reference material.

This document reflects the current privacy posture of BubblesTheDev Web Browser version `1.1.33`.
...
| Automatic diagnostics upload | No by default; optional privacy-safe severe-event reporting only when enabled by the user | Yes | Yes | Limited | Limited | Limited | Limited | Limited |
| Cloud sync requirement | None | Optional | Optional | Optional | Optional | Optional | Optional | Optional |
| Auto-updater service | Background update checks and downloads with a user-visible installer and restart step; not a fully silent hidden auto-updater service | Present | Present | Present | Present | Present through OS updates | Present | Present |
| Built-in multilingual interface support | Yes, with `679` locale packs and `453` base language families, plus local-only language handling, runtime language switching, and locale-aware formatting | Yes, with `100+` interface languages | Yes, with `84` display languages | Yes, with `100+` estimated | Yes, with `97` locales / `88` languages | Yes, with `90+` estimated system-supported languages | Yes, with `100+` estimated | Yes, with `80+` estimated |
...
| Ad and tracker filtering built in | Yes | Limited | Limited | Yes | Limited | Limited | Yes | Yes |
...
| Split-view browsing built in | Yes | Varies | Yes | Yes | Yes | Varies | Yes | Yes |
...
As of version `1.1.33`, the supported services are Disney+, Hulu, Max, Netflix, Paramount+, Prime Video, Apple TV+, AMC+, Peacock, Crunchyroll, YouTube TV, Sling TV, Pluto TV, The Roku Channel, Plex, Discovery+, ESPN+, MGM+, STARZ, and Tubi.
...
* the isolated Streaming Hub in `1.1.16` adds service-specific session separation and popup hardening without changing the browser into a credential interceptor or telemetry client
* lighter performance-summary handling and deferred startup work in `1.1.16` reduce repeated local metrics work during normal browsing without changing the browser's privacy posture
* the expanded accessibility settings and smoother `AI & Diagnostics` panel behavior in `1.1.16` stay local to the device and do not add built-in telemetry or analytics collection
...
Version `1.1.16` keeps the same local-first privacy direction described above while carrying forward the local AI and diagnostics layer and the broader accessibility, startup, installer, and media-tool refinements introduced in `1.1.15`.

Important privacy-facing differences in `1.1.16` compared with the older `1.0.65` baseline described above:
...
* installed update behavior can create or refresh a Desktop folder named `BubblesTheDev - WebBrowser Update Notes` so the current bundled release notes stay user-visible locally

## Additional High-Level Comparison Notes For 1.1.16

If the comparison table above were extended for `1.1.16`, the Bubbles column would also include the following clarifications:
...
Version `1.1.16` keeps the optional on-device AI layer in place.
...
The older sections above describe diagnostics as local-only manual export behavior. Version `1.1.16` keeps that extended model user-controlled.
...
The broad privacy conclusion from the original comparison still holds in `1.1.16`:
