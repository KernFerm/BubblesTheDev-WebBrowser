# Project Updates

Project news and community update posts are shared in the official Discord server.

## Discord

[BubblesTheDev Web Browser Discord](https://discord.gg/eYBzdZVbz6)

---

## Current Release

- Version `1.1.30` is the current release.
- Older versions are not supported for current fixes and improvements.

Current documentation highlights for this release:

- the browser now has a large multilingual foundation with `679` supported locale packs and `453` base language families
- localization startup and UI language switching were profiled and tuned in the live app
- the browser keeps its local-first privacy posture with no built-in telemetry or analytics

For the latest browser announcements, release posts, and community updates, check the Discord server above.
